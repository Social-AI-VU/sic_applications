"""
Pepper microphone -> Google STT -> LangGraph agent -> Pepper MCP tools.
"""
...
class PepperMcpVoiceDemo(PepperMcpSICApplication):
    def __init__(
        self,
        *,
        model: str,
    ...
    async def _async_mcp_loop(
        ...
        prior_len = 0
        self.logger.info(
            "Listening on the Pepper mic (via MCP). Speak a command; Ctrl+C to stop.\n"
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Pepper mic + Google STT (MCP server) + LangGraph agent."
    )
    ...
    parser.add_argument("--thread-id", type=str, default="pepper-mcp-voice")
    ...
    if not args.robot_ip or not str(args.robot_ip).strip():
        print("Missing Pepper IP (--robot-ip or ROBOT_IP / PEPPER_IP).", file=sys.stderr)
        raise SystemExit(1)
