...
class NaoVoiceDetectionDemo(SICApplication):
    ...
    def on_voice_detection(self, message: VoiceDetectionMessage):
        ...
        if message.is_speaking:
            self.logger.info("Someone is speaking! (speech proportion: {:.2f})".format(message.speech_proportion))
        else:
    ...
    def setup(self):
        ...
        voice_detection_conf = VoiceDetectionConf(sampling_rate=16000)  # NAO's microphone sample rate
        self.voice_detection = VoiceDetection(input_source=nao_mic, conf=voice_detection_conf)
        
        time.sleep(1)
        
        self.voice_detection.register_callback(self.on_voice_detection)
        self.logger.info("Voice Detection service is ready. Start speaking to see detection results!")
    
    def run(self):
        """Main application loop."""
        self.logger.info("Starting NAO Voice Detection Demo")
        self.logger.info("The demo will continuously monitor for speech activity using NAO's microphone.")
