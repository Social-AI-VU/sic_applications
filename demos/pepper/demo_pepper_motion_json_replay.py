...
class PepperMotionReplayDemo(SICApplication):
    ...
    def _load_motion_from_json(self):
        ...
        try:
            ...
            for joint, values in data["motion"].items():
                ...
                recorded_angles.append(
                    [a / precision_factor_angles for a in values["angles"]]
                )
                recorded_times.append(
                    [t / precision_factor_times for t in values["times"]]
                )

            # Wrap the data in a NaoqiMotionRecording object
            self.recording = NaoqiMotionRecording(
                recorded_joints, recorded_angles, recorded_times
            )

            self.logger.info(
                "Motion loaded: {} joints, {} keyframes".format(
                    len(recorded_joints),
                    len(recorded_angles[0]) if recorded_angles else 0,
                )
            )
    ...
    def run(self):
        ...
        try:
            ...
            self.pepper.stiffness.request(
                Stiffness(
                    stiffness=0.7, joints=self.chain, enable_joint_list_generation=False
                )
            )
    ...
    def shutdown(self):
        ...
        try:
            ...
            self.pepper.stiffness.request(
                Stiffness(
                    stiffness=0.0, joints=self.chain, enable_joint_list_generation=False
                )
            )
