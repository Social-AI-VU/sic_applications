...
class PepperPuppeteeringDemo(SICApplication):
    ...
    def setup(self):
        ...
        puppet_conf = PepperMotionStreamerConf(
            samples_per_second=self.stream_hz, stiffness=0.0
        )
        performer_conf = PepperMotionStreamerConf(
            samples_per_second=self.stream_hz, stiffness=1.0
        )
        ...
        self._blockage_thr = threading.Thread(
            target=self._blockage_watchdog, daemon=True
        )
    ...
    def _initialise_robots(self):
        ...
        self.performer_motion = self.performer.motion_streaming(
            input_source=self.puppet_motion
        )
    ...
    def _say(self, text: str) -> None:
        ...
        """
        self.puppet.tts.request(NaoqiTextToSpeechRequest(text, language="English"))
    ...
    def _lock_joint(self, chain: str):
        ...
        try:
            ...
            self.logger.info(
                f"Added {chain} to locked joints set: {self._locked_joints}"
            )
            ...
            individual_joints = (
                ["HeadYaw", "HeadPitch"] if chain == "Head" else CRITICAL_JOINTS[chain]
            )
            self.logger.info(f"Getting angles for joints: {individual_joints}")
            angles_response = self.puppet.motion.request(
                NaoqiGetAnglesRequest(individual_joints, True)
            )
            ...
            self.logger.info(
                f"Setting angles on both robots: {dict(zip(individual_joints, angle_values))}"
            )
    ...
    def _unlock_joint(self, chain: str):
        ...
        try:
            ...
            self.puppet.stiffness.request(
                Stiffness(0.0, joints=phys, enable_joint_list_generation=False)
            )
            self.performer.stiffness.request(
                Stiffness(1.0, joints=phys, enable_joint_list_generation=False)
            )
    ...
    def _on_back_bumper(self, msg) -> None:
        ...
        """
        self.logger.info(
            f"Back bumper pressed: value={msg.value}, paused={self._paused}"
        )
    ...
    def _start_velocity_monitor(self):
        ...
        self._vel_mon_thr = threading.Thread(target=self._velocity_loop, daemon=True)
        ...
        while not self._vel_mon_stop.is_set():
            try:
                ...
                if v_mag < VEL_THR and not self._prot_off:
                    ...
                    self._prot_off = True
                    self.logger.info(
                        f"Velocity: ({vx:.3f}, {vy:.3f}, {vth:.3f}) - Collision protection disabled"
                    )
                    ...
                    self.logger.info(
                        f"Velocity: ({vx:.3f}, {vy:.3f}, {vth:.3f}) - Collision protection enabled"
                    )
        ...
        while not self._blockage_stop.is_set():
            ...
            try:
                with self._blockage_lock:
                    response = self.performer.motion.request(
                        NaoqiGetAnglesRequest(names, True)
                    )
    ...
    def shutdown(self) -> None:
        ...
        for chain in self.active_joints:
            try:
                self.puppet.stiffness.request(
                    Stiffness(0.7, joints=[chain], enable_joint_list_generation=False)
                )
                self.performer.stiffness.request(
                    Stiffness(0.7, joints=[chain], enable_joint_list_generation=False)
                )
                ...
                self.logger.warning(
                    "Error resetting stiffness for {}: {}".format(chain, e)
                )

        # Restore puppet stiffness before rest (backup method)
        self.puppet.stiffness.request(
            Stiffness(
                0.7, joints=self.active_joints, enable_joint_list_generation=False
            )
        )
